...
secrets:
  cloudflare_api_key:
    file: "${configs_folder}/secrets/cloudflare_api_key.secret"
So far - I just made a flat file at ${configs_folder}/secrets/cloudflare_api_key.secret with the CF_API_KEY environment variable VALUE ONLY.
...
# Host Path: ${configs_folder}/traefik/traefik-static.yaml
...
All the rest of these files are in ${configs_folder}/traefik/dynamic/
...
# Host Path: ${configs_folder}/traefik/dynamic/static-servers.yaml
...
http:
  services:
    ...
    ${var_nas_name}:
      loadBalancer:
        servers:
          - url: "http://${var_nas_ip}:8880"
I have two services so far - noop and ${var_nas_name} (shout out to my NAS!)
Now a piece I largely believe unnecessary, but demonstrative, is the static-routers.yml
# Traefik Dynamic Configuration
# Routers: Static (Non-Docker) Routers
# Host Path: ${configs_folder}/traefik/dynamic/static-routers.yaml
...
http:
  routers:
    gkoerk:
      ...
      middlewares:
        - forward-auth
      service: ${var_nas_name}
      rule: "Host(`${var_domain}`)"
      tls:
        certResolver: ${var_certresolver}
        domains:
          - main: "*.${var_domain}"
            sans:
              - ${var_domain}
    ...
    ${var_nas_name}:
      ...
      middlewares:
        - forward-auth
      service: ${var_nas_name}
      rule: "Host(`${var_nas_name}.${var_domain}`)"
      tls:
        certResolver: ${var_certresolver}
        domains:
          - main: "*.${var_domain}"
...
# Host Path: ${configs_folder}/traefik/dynamic/basic-user.yaml
...
# Host Path: ${configs_folder}/traefik/dynamic/compress.yaml
...
# Host Path: ${configs_folder}/traefik/dynamic/https-redirect.yaml
  ...
  middlewares:
    https-redirect:
      redirectScheme:
        ...
        # Host Path: ${configs_folder}/traefik/dynamic/forward-auth.yaml
        ...
        http:
          middlewares:
            forward-auth:
              forwardAuth:
                address: "http://${var_nas_ip}:8080/authorize"
        ...
        # Host Path: ${configs_folder}/traefik/dynamic/secured-chain.yaml
        ...
        # Host Path: ${configs_folder}/traefik/dynamic/globalredirect.yaml
        ...
        traefik_app (in traefik.yml) labels:
              labels:
                ...
                - "traefik.http.routers.traefik.rule=Host(`traefik.${var_domain}`)"
